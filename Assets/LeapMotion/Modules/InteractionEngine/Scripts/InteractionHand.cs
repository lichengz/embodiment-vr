...
namespace Leap.Unity.Interaction
{
    ...
    public class InteractionHand : InteractionController
    {
        ...
        //Let's define our own "grabbing" mechanism
        void Update()
        {
            if (gameObject.name.ToString() == "Interaction Hand (Right)")
            {
                testTransform = leapHand.Fingers[0].TipPosition.ToVector3();
                float pitchAngle = _leapProvider.CurrentFrame.Hands[0].Direction.Pitch * Mathf.Rad2Deg;
                float yawAngle = _leapProvider.CurrentFrame.Hands[0].Direction.Yaw * Mathf.Rad2Deg;
                float rollAngle = _leapProvider.CurrentFrame.Hands[0].PalmNormal.Roll * Mathf.Rad2Deg;
                if (tool != null) tool.GetComponent<Tool>().pitch = pitchAngle;
                if (tool != null) tool.GetComponent<Tool>().yaw = yawAngle;
                if (tool != null) tool.GetComponent<Tool>().roll = rollAngle;
                //DetectGrabbing();
                //DetectGrabbingByThumb();
                //===========================================Switch to Leap Motion=========================================
                if (isUsingLeapMotion) DetectGrabbingNew();
            }

        }
        // use 3 fingers to detect grabbing
        // void DetectGrabbing()
        // {
        //     float threshlod = 0.06f;
        //     float avgDistanceFromPalm = 0f;
        //     for (int i = 1; i < 3; i++)
        //     {
        //         if (testPalm != null)
        //         {
        //             Vector3 tmpVec = leapHand.Fingers[i].TipPosition.ToVector3() - testPalm.position;
        //             avgDistanceFromPalm += tmpVec.magnitude;
        //         }
        //     }
        //     avgDistanceFromPalm /= 4;
        //     if (avgDistanceFromPalm < min) min = avgDistanceFromPalm;
        //     if (avgDistanceFromPalm > max) max = avgDistanceFromPalm;
        //     if (avgDistanceFromPalm < threshlod)
        //     {
        ...
        // use the thumb to detect grabbing
        float avgDistanceFromPalm = 0f;
        float avgDistanceFromPalm2 = 0f;
        float oldAvgDistanceFromPalm = 0f;
        float oldAvgDistanceFromPalm2 = 0f;
        float checkRate = 0.1f;
        float nextCheck = 0f;
        void DetectGrabbingByThumb()
        {
            float threshlod = 0.05f;

            //if (testPalm != null)

            Vector3 tmpVec = leapHand.Fingers[0].TipPosition.ToVector3() - leapHand.Fingers[2].TipPosition.ToVector3();
            //Vector3 tmpVec = leapHand.PalmPosition.ToVector3() - leapHand.Fingers[2].TipPosition.ToVector3();
            avgDistanceFromPalm = tmpVec.magnitude;
            Vector3 tmpVec2 = leapHand.Fingers[0].TipPosition.ToVector3() - leapHand.Fingers[1].TipPosition.ToVector3();
            //Vector3 tmpVec2 = leapHand.PalmPosition.ToVector3() - leapHand.Fingers[1].TipPosition.ToVector3();
            avgDistanceFromPalm2 = tmpVec2.magnitude;
            //
            if (Time.time > nextCheck)
            {
                nextCheck = Time.time + checkRate;
                if (Math.Abs(avgDistanceFromPalm2 - oldAvgDistanceFromPalm2) > 0.03f)
                {
                    if (tool != null) tool.GetComponent<Tool>().isLock = true;
                }
                else
                {
                    if (tool != null) tool.GetComponent<Tool>().isLock = false;
                }
                oldAvgDistanceFromPalm2 = avgDistanceFromPalm2;
            }


            if (avgDistanceFromPalm < min) min = avgDistanceFromPalm;
            if (avgDistanceFromPalm > max) max = avgDistanceFromPalm;
            if (avgDistanceFromPalm < threshlod || avgDistanceFromPalm2 < threshlod)
            {
                if (grabber != null) grabber.GetComponent<GrabberDetect>().isGrabbing = true;
            }
            else
        ...
        void DetectGrabbingNew()
        {
            // float onThreshold = 0.05f;
            // float offThreshold = 0.1f;
            float onThreshold = 0.04f;
            float offThreshold = 0.08f;
            //middle finger
            Vector3 tmpVec1 = leapHand.Fingers[0].TipPosition.ToVector3() - leapHand.Fingers[2].TipPosition.ToVector3();

            float distance1 = tmpVec1.magnitude;
            //index finger
            Vector3 tmpVec2 = leapHand.Fingers[0].TipPosition.ToVector3() - leapHand.Fingers[1].TipPosition.ToVector3();
            float distance2 = tmpVec2.magnitude;
            float distance = Math.Min(distance1, distance2);
            //scoreBoardText.GetComponent<TextMesh>().text = distance.ToString();
            if (grabber != null) grabber.GetComponent<GrabberDetect>().force = distance;

            //isLock
            if (Time.time > nextCheck)
            {
                nextCheck = Time.time + checkRate;
                if (Math.Abs(distance - oldAvgDistanceFromPalm2) > 0.02f)
                {
                    if (tool != null) tool.GetComponent<Tool>().isLock = true;
                }
                else
                ...
                oldAvgDistanceFromPalm2 = distance;
            }


            if (distance < onThreshold)
            {
                if (grabber != null) grabber.GetComponent<GrabberDetect>().isGrabbing = true;
            }
            if (distance > offThreshold)
            {
                if (grabber != null) grabber.GetComponent<GrabberDetect>().isGrabbing = false;
            }
        }
